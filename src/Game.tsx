...
function Game() {
  ...
  const FUTURE_BLOCKS_COUNT = 2; // Cuántos bloques futuros pre-generar (actual + booster + 1 de reserva) (de tu compañero
      ...
      if (anyCombinationOccurred) {
        if (comboCountForThisStep === 1) {
          ...
          if (firstCombination && firstCombination.args[3] >= 3) {
            showTemporaryMessage(setComboMessage, `COMBO x${firstCombination.args[3]}`, COMBO_DISPLAY_DURATION);
          } else {
            setComboMessage(null);
          }
        } else {
